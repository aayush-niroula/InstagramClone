...
function CreatePost({ open, setOpen }) {
  ...
  const createPostHandler = async (e) => {
      ...
      if(imagePreview){
        formData.append("image",file)    // in route we give post name as image ...
      }
      try {
        setLoading(true)
        ...
        <Button onClick={() => imageRef.current.click()} className='w-fit mx-auto bg-[#0095f6] hover:bg-[#3569c8]'>Select from device</Button>
        {
          imagePreview && (
            loading ?(
              ...
              <Button onClick={createPostHandler} type='submit ' className='w-full bg-blue-500'>Post</Button>
            )
          )
        }
